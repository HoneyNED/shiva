"""This module has been modified to add SMTP authentication support to
SHIVA.
The code to add SMTP Authentication has been taken from secure_smtpd library
written by Benjamin E. Coe. https://github.com/bcoe/secure-smtpd
"""
...
class SMTPChannel(asynchat.async_chat):
    ...
    def smtp_AUTH(self, arg):
        if arg is None:
            self.push('535 invalid authentication')
            return 
...
class SMTPServer(asyncore.dispatcher):
    def __init__(self, localaddr, remoteaddr, require_authentication=False, credentials={'username':'', 'password':''}, validate_rcpt=False, rcpt_hosts=None):
        ...
        if require_authentication:
            self.credential_validator = CredentialValidator(credentials['username'], credentials['password'])
        else:
